...
export default function AllocationPage({ userName = "Priya", onNavigate, onLogout }) {
  ...
  return (
    <div className="min-h-screen bg-[#060a10] font-['Inter'] flex items-center justify-center p-6">
      <div className="w-full h-[calc(100vh-3rem)] rounded-2xl border border-[#232C36] bg-[#0A0E13] overflow-hidden flex fade-in">
        <Sidebar active={activeNav} onSelect={goTo} />

        <div className="flex-1 min-w-0 h-full overflow-y-auto custom-scroll p-8 max-w-[560px]">
      ...
      <style>{`
        ...
        .custom-scroll::-webkit-scrollbar-thumb { background: #232C36; border-radius: 999px; }
        .custom-scroll::-webkit-scrollbar-thumb:hover { background: #3A4551; }
        .custom-scroll { scrollbar-width: thin; scrollbar-color: #232C36 transparent; }
      `}</style>
    </div>
  );
}
